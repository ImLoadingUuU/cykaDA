...
<head>
  ...
  $stmt = $connect->prepare("SELECT * FROM da_clients WHERE username = ? AND password = ?");
  ...
  if ($result->num_rows == 0) {
    ...
    $_SESSION["email"] = null;
    $_SESSION["message"] = "Login Required - Your password may be reseted";
    header("Location: ./login.php");
  }
  ?>
  <style>
...
<header class="navbar navbar-expand-md navbar-light d-print-none">
  <div class="container-xl">
    ...
    <h1 class="navbar-brand navbar-brand-autodark d-none-navbar-horizontal pe-0 pe-md-3">
      <a href=".">
        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-server-2" width="44" height="44"
    ...
    <div class="navbar-nav flex-row order-md-last">
      ...
      <div class="nav-item dropdown">
        <a href="#" class="nav-link d-flex lh-1 text-reset p-0" data-bs-toggle="dropdown" aria-label="Open user menu">
                <span class="avatar avatar-sm" style="background-image: url(<?php
                echo $grav_url = "https://www.gravatar.com/avatar/" . md5(strtolower(trim($_SESSION["email"]))) . "?s=40";
                ?>)"></span>
          <div class="d-none d-xl-block ps-2">
            <div><?php echo $_SESSION["username"] ?></div>
            <div class="mt-1 small text-muted"><?php echo $_SESSION["email"] ?></div>
          </div>
        </a>
